...
print("CERAD score distribution:")
...
individuals_main, individuals_val = train_test_split(individuals, test_size=0.2, stratify=CERAD_scores, random_state=42)
CERAD_scores_main = np.array([ceradDict[ind] for ind in individuals_main])
...
for fold, (train_idx, test_idx) in enumerate(skf.split(individuals_main, CERAD_scores_main)):
    print(f"Fold {fold + 1}\n-------------------------------")
    ...
    # Feature selection
    _, index = selectKBest(X_train.X, y_trainCerad, top_k=1000)
    ...
    # print the division of the braakscore
    print("Train set CERAD score distribution:")
    dist = pd.Series(y_trainCerad).value_counts()
    print(dist)
    ...
    y_trainMTL_braak = [y_train_coral_cerad, y_train_coral_braak]
    y_testMTL_braak = [y_test_coral_cerad, y_test_coral_braak]
    y_valMTL_braak = [y_val_coral_cerad, y_val_coral_braak]

    y_trainMTL_cogdx = [y_train_coral_cerad, y_train_coral_cogdx]
    y_testMTL_cogdx = [y_test_coral_cerad, y_test_coral_cogdx]
    y_valMTL_cogdx = [y_val_coral_cerad, y_val_coral_cogdx]

    y_trainMTL_sex = [y_train_coral_cerad, y_trainSex]
    y_testMTL_sex = [y_test_coral_cerad, y_testSex]
    y_valMTL_sex = [y_val_coral_cerad, y_valSex]

    y_train_MTLage = [y_train_coral_cerad, y_trainAge]
    y_test_MTLage = [y_test_coral_cerad, y_testAge]
    y_valMTL_age = [y_val_coral_cerad, y_valAge]

    y_train_MTLapoe = [y_train_coral_cerad, y_trainApoc]
    y_test_MTLapoe = [y_test_coral_cerad, y_testApoc]
    y_valMTL_apoe = [y_val_coral_cerad, y_valApoc]
    ...
    # Create dataloaders
    train_loaderCERAD_coral, val_loaderCERAD_coral, test_loaderCERAD_coral = dataLoader(
        X_train, X_val, X_test,
        y_train_coral_cerad, y_val_coral_cerad, y_test_coral_cerad,
        types=[f],
    ...
    modelCerad = CoralOrdinalNet(input_dim=1000, num_classes=4)
    modelMTLBraak = MultiTaskModel([4, 4], ["coral", "coral"])
    ...
    optimizerBraak = torch.optim.Adam(modelCerad.parameters(), lr=learningRate, weight_decay=weightDecay)
    ...
    valLosses = []
    accs = []
    CERADBraakacc, CERADBraakpm1acc = [], []
    CERADCogdxacc, CERADCogdxpm1acc = [], []
    CERADAgeacc, CERADAgepm1acc = [], []
    CERADSexacc, CERADSexpm1acc = [], []
    CERADApoeacc, CERADApoepm1acc = [], []
    ...
    for t in range(epochs):
        print(f"Epoch {t + 1}\n-------------------------------")
        print("Training model for CERAD score (coral)")
        train(train_loaderCERAD_coral, modelCerad, loss_fnBraak_coral, optimizerBraak)
        _, _, _, idsCERAD = test(test_loaderCERAD_coral, modelCerad, loss_fnBraak_coral, ordinal=True)
        valLossCERAD, _, _, idsCERADVal = test(val_loaderCERAD_coral,   modelCerad, loss_fnBraak_coral, ordinal=True)
        ...
        with open(f"CERADIds/IDs{fold}", "w") as f:
    ...
    del modelMTLcogdx, modelMTLsex, modelMTLAge, modelMTLApoe
